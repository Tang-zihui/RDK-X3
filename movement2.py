...
def stop(speed,state,ser):     #停止，需要传现在的速度和行进方向，防止电机瞬间减速不稳定
...
def back(speed,tim,ser):       #后退 但是传参speed 速度为正值
...
def servo1(now,to,ser):
    if(now<to):
        for angle in range (now,to+SERVO_STEP,SERVO_STEP):   #angle 表示希望转到的角度
            s_order=Servo_order_gen(angle,SERVO1)
            s_order=bytes.fromhex(s_order)
            write_num=ser.write(s_order)          #写串口
            time.sleep(0.01)
            print(s_order.hex())
    else:
...
def servo2(now,to,ser):
    if(now<to):
        for angle in range (now,to+SERVO_STEP,SERVO_STEP):   # 能否为负数
        ...
        for angle in range (now,to-SERVO_STEP,-SERVO_STEP):   
            s_order=Servo_order_gen(angle,SERVO2)  
            s_order=bytes.fromhex(s_order)  
            write_num=ser.write(s_order)          #写串口
            time.sleep(0.01)
...
def servo4(now,to,ser):
    if(now<to):
        ...
        for angle in range (now,to-SERVO_STEP,-SERVO_STEP):  
            ...
            time.sleep(0.01)
            print(s_order.hex())
def motor1(tim,dir,ser):    #直接使用上位机引脚控制 ，需要确定正反转
...
def adjust(model,cap,target,ser):
    pass
